...
EspPlayersBox:AddToggle("Box2D", {
    ...
    Callback = function(state)
        pcall(function()
            ...
            local function clearESP(character)
                local obj = box2dCache[character]
                if obj then
                    obj:Destroy()
                    box2dCache[character] = nil
                end
            end

            local function applyESP(player, character, color)
                local hrp = character:FindFirstChild("HumanoidRootPart")
                if not hrp then return end

                if box2dCache[character] then
                    return
                end
                ...
                gui.Parent = hrp
                box2dCache[character] = gui
            end

            if state then
                local playerList = Players:GetPlayers()

                box2dConnections.loop = task.spawn(function()
                    while box2dESP do
                        task.wait(1)
                        ...
                        for _,player in ipairs(playerList) do
                            if player ~= LocalPlayer and player.Character then
                                ...
                                applyESP(player, player.Character, color)
                            end
                        end
                    end
                end)

                box2dConnections.added = Players.PlayerAdded:Connect(function(p)
                    table.insert(playerList, p)
                end)

                box2dConnections.removing = Players.PlayerRemoving:Connect(function(p)
                    if p.Character then
                ...
                for char,obj in pairs(box2dCache) do
                    obj:Destroy()
                end
                table.clear(box2dCache)

                for _,c in pairs(box2dConnections) do
                ...
                table.clear(box2dConnections)
            end
        end)
    end
})

local box3dESP = false
local box3dCache = {}
...
EspPlayersBox:AddToggle("Box3D", {
    ...
    Callback = function(state)
        pcall(function()
            ...
            local function clearESP(character)
                local obj = box3dCache[character]
                if obj then obj:Destroy() box3dCache[character]=nil end
            end

            local function applyESP(player, character, color)
                if box3dCache[character] then return end
                ...
                box3dCache[character] = box
            end

            if state then
                local playerList = Players:GetPlayers()

                box3dConnections.loop = task.spawn(function()
                    while box3dESP do
                        task.wait(1)

                        for _,player in ipairs(playerList) do
                            if player ~= LocalPlayer and player.Character then
                                applyESP(player, player.Character, Color3.fromRGB(0,255,0))
                            end
                        end
                    end
                end)
                ...
                for char,obj in pairs(box3dCache) do obj:Destroy() end
                table.clear(box3dCache)
            end
        end)
    end
})
...
EspPlayersBox:AddToggle("Skeleton", {
    ...
    Callback = function(state)
        pcall(function()
            ...
            local function clearESP(character)
                local objs = skeletonCache[character]
                if objs then
                    for _,v in pairs(objs) do v:Destroy() end
                    skeletonCache[character]=nil
                end
            end

            local function applyESP(player, character)
                if skeletonCache[character] then return end
                ...
                for _,part in ipairs(character:GetChildren()) do
                    if part:IsA("BasePart") then
                        ...
                        table.insert(objs, att)
                    end
                end

                skeletonCache[character] = objs
            end

            if state then
                local playerList = Players:GetPlayers()

                skeletonConnections.loop = task.spawn(function()
                    while skeletonESP do
                        task.wait(1)
                ...
                for char,objs in pairs(skeletonCache) do
                    for _,v in pairs(objs) do v:Destroy() end
                end
                table.clear(skeletonCache)
            end
        end)
    end
})
...
EspPlayersBox:AddToggle("Tracer", {
    Text = "Tracer",
    Default = false,
    Callback = function(state)
        pcall(function()
            ...
            local function clearESP(character)
                local obj = tracerCache[character]
                if obj then
                    for _,v in pairs(obj) do v:Destroy() end
            ...
            local function applyESP(player, character)
                ...
                if tracerCache[character] then return end
                ...
                tracerCache[character] = {beam,a0,a1}
            end

            if state then
                local playerList = Players:GetPlayers()

                tracerConnections.loop = task.spawn(function()
                    while tracerESP do
                        task.wait(0.1)
                        ...
                        for _,data in pairs(tracerCache) do
                            data[2].WorldPosition = Camera.CFrame.Position
                        end
                    end
                end)

            else
                for char,data in pairs(tracerCache) do
